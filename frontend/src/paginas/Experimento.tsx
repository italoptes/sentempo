// paginas/Experimento.tsx — Preparação e execução de uma rodada (rota /experimento/:tempo/:condicao)
...
export function Experimento({ participanteId, onRecarregar }: Props) {
  const navigate = useNavigate();
  const { tempo: tempoParam, condicao: condicaoParam } = useParams<{ tempo: string; condicao: string }>();

  const tempoAlvoMs = Number(tempoParam);
  const condicao = condicaoParam as Condicao;

  const { estado, mensagemErro, pulseCount, iniciarRodada, finalizarRodada, limpar } = usarExperimento(participanteId);
    ...
    <div className="min-h-screen bg-fundo flex flex-col pagina-entrar">
      <header className="bg-principal shadow-sm">
        <div className="max-w-lg mx-auto px-4 py-4 flex items-center gap-3">
          ...
          <span className="text-destaque-claro text-sm font-medium">Preparação</span>
        </div>
      </header>
